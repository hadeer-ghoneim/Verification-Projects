...
class SPI_Wrapper_test extends uvm_test;
    ...
    function void build_phase(uvm_phase phase);
        ...
        env_wrapper = SPI_Wrapper_env::type_id::create("env_wrapper", this);  
        env_slave = spi_slave_env::type_id::create("env_slave", this); 
        ...
        if (!uvm_config_db#(virtual spi_slave_if)::get(this, "", "spi_slave_if", cfg_slave.if_slave))  
            `uvm_fatal(get_type_name(), "SPI slave if not found");
    
        uvm_config_db#(config_slave)::set(this, "*", "GFG_slave", cfg_slave);  
        uvm_config_db#(config_wrapper)::set(this, "*", "cfg", cfg_wrapper);  
        uvm_config_db#(RAM_config_obj)::set(this, "*", "CFG", cfg_ram);  

        // Call display if needed
    ...
    task run_phase(uvm_phase phase);
        ...
        // Run reset sequence on correct sequencer (env_wrapper.agt.seq_wrapper)
        seq_rst_wrapper.start(env_wrapper.agt.seq_wrapper);  
        `uvm_info(get_type_name(), "Reset sequence started", UVM_MEDIUM);
        
        // Run main sequence
        seq_wrapper.start(env_wrapper.agt.seq_wrapper);  
