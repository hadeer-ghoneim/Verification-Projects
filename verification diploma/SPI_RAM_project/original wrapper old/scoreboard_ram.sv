        ...
        forever begin
           ...
           // golden_model(item);  // Calculate expected outputs
            ...
            if (item.dout_ref=== item.dout && item.tx_valid_ref === item.tx_valid) begin
                ...
                error_count++;
                `uvm_error("SCOREBOARD", $sformatf("Mismatch! din=0x%0h rx_valid=%0b rst_n=%0b\nExpected: dout=0x%0h tx_valid=%0b\nActual:   dout=0x%0h tx_valid=%0b", 
                          item.din, item.rx_valid, item.rst_n, item.dout_ref, item.tx_valid_ref, item.dout, item.tx_valid))
            end
        end    
    endtask 
        ...
        if (!item.rst_n) begin
            ...
            case (item.din[9:8])
                ...
                2'b11: begin  // Read data
                    ...
                    expected_data = mem[data];
                    tx_valid_expected = 1;
                    
                    // Update state for next cycle
                    current_dout = mem[data];
                    current_tx_valid = 1;
                end
            endcase
        end
